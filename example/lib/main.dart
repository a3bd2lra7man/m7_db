...
class AppDB extends M7DB{
  ...
  FutureOr<void> onCreate(Database db, int version)async {
    ...
    await db.execute(createTableStatement(tableName: 'User',fields: 'id INTEGER PRIMARY KEY AUTOINCREMENT , name TEXT ,isCompleted INTEGER,date INTEGER')) ;
...
class User extends M7Table{
  ...
  M7Table copyWith({String name,bool isCompleted,DateTime dateTime}) {
    return User(id: this.id,name: name ?? this.name,isCompleted: isCompleted ?? this.isCompleted,date: dateTime ?? this.date);
  }
